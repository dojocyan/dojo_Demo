...
#define DojoCHAIN_SEED_NONCE  0x0539a019ca550825ULL
...
//    printf("%s: SEND %s\n", ToString(you).c_str(), pszCommand); 
  ...
  void PushVersion() {
    ...
    uint64 nLocalNonce = DojoCHAIN_SEED_NONCE;
    int64 nLocalServices = 0;
    MCAddress me(MCService("0.0.0.0"));
    BeginMessage("version");
    int nBestHeight = GetRequireHeight();
    string ver = "/Dojochain-seeder:0.01/";
...
/*
int main(void) {
  MCService ip("Dojochain.sipa.be", 8333, true);
  vector<MCAddress> vAddr;
  vAddr.clear();
  int ban = 0;
