...
IxDojoChainBridge::IxDojoChainBridge()
{
	m_pRootKey = NULL;
...
void IxDojoChainBridge::Initialize(NETWORK_TYPE eNetworkType)
{
...
void IxDojoChainBridge::Release()
{
	bool hasstartecc = ECC_HasStarted();
	if (hasstartecc)
	{
...
bool IxDojoChainBridge::GetExtKeyWif(MCExtKey* pExtKey, char* pOutWif, int iSize)
{
...
MCExtKey IxDojoChainBridge::ImportExtKey(const char* pExtKeyWif)
{
	std::string strK = pExtKeyWif;
    DojoChainExtKey* pCLEK = new DojoChainExtKey(strK);
	MCExtKey kCEK = pCLEK->GetKey();
	delete pCLEK;
	return kCEK;
}

MCKey* IxDojoChainBridge::GetCellKey(MCExtKey* pCEK)
{
	if (pCEK == NULL)
	{
		return NULL;
...
bool IxDojoChainBridge::GetKeyWif(MCKey* pKey, char* pOutWif, int iSize)
{
	...
	std::string strK = DojoChainSecret(*pKey).ToString();
	if (iSize <= strK.length())
	{
		return false;
	}
...
MCKey IxDojoChainBridge::ImportKey(const char* pWif)
{
	DojoChainSecret kCS;
...
bool IxDojoChainBridge::GetAddress(MCKey* pKey, char* pOutWif, int iSize)
{
	if (pKey == NULL || pOutWif == NULL)
	{
		return false;
	...
	std::string strK = DojoChainAddress(kCKI).ToString();
...
bool IxDojoChainBridge::CreateRootExtKey(const char* pAid)
{
...
MCExtKey* IxDojoChainBridge::GetRootExtKey()
{
	return m_pRootKey;
}

void IxDojoChainBridge::InitializeRPCInfo(const char* pHost, const char* pPort, const char* pUser, const char* pPwd)
{
	m_strHost = pHost;
	m_strPort = pPort;
	m_strUser = pUser;
...
void IxDojoChainBridge::ResetArgs()
{
...
float IxDojoChainBridge::GetBalance(const char* pAddress)
{
	if (pAddress == NULL)
	{
		return -1.0f;
...
bool IxDojoChainBridge::Transfer(const char* pFromPirKeyWif, const char* pDestAddr, float fAmount, const char* pChangeAddr)
{
