#!/bin/bash
# Copyright (c) 2020 The DojoChain Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete DojoChain database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no DojoChain datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old DojoChain datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected DojoChain 0.7 datadir."
    ;;
  3)
    echo "Detected DojoChain pre-0.8 datadir."
    ;;
  4)
    echo "Detected DojoChain 0.8 datadir."
    ;;
esac
